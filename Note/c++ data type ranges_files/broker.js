...
C=D[E].f;}}}return F;},isMatchContent:function(O){var I=true,D=0;while(I&&D<O.length){var L=false;
var K=false;var J=O[D];if(J.element){var F=document.getElementsByTagName(J.element);
var G=true;for(var E=0;E<F.length;E++){var N=new RegExp(J.elementValue);if(N){if(N.test(F[E].innerHTML)){if(G){G=false;
}L=true;}}else{L=true;}if(J.attrib&&J.attrib.length){var H=F[E].attributes.getNamedItem(J.attrib);
var M=new RegExp(J.attribValue);if(H){if(J.attribValue&&J.attribValue.length){if(M.test(H.value)){K=true;
}}else{K=true;}}}else{K=true;}}}if(!L||!K){I=false;}D++;}return I;},isMatchCookie:function(E){var I=true,G=0;
...
}else{return;}},start:function(){this.init();},run:function(){this.config.Events.beforeRecruit();
